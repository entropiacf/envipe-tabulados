---
title: "Total de Víctimas ENVIPE"
...
que es la que contiene. Al correr la función `head()` nos dará los primeros n
registros que le pidamos, eso ayuda a visualizar si los datos se cargaron 
adecuadamente.

```{r}

#df <- read_csv( here('/home/isaias/entropia-codigo/envipe-tabulados/datos/datos-originales/TPer_Vic2.csv') )
...
# Observé que el factor de expansión del hogar viene en caracter para la base de 2024
# por ello lo convierto a entero.

df$fac_hog <- as.integer(df$fac_hog)

```

> Volvemos a mostrar los primeros registros
...
## 4. Procesamiento de datos (Responder nuestra pregunta)
...
preg <- c('AP6_4_01',  # Sobre el Robo total de vehículo (automóvil, camioneta o camión)
          ...
          #'AP6_4_03', # Sobre la Pinta de barda o grafiti en su casa, *NO* se debe incluir
...
> ## ¿Cuántos hogares se estima que fueron victimizados en 2022 en Mexico?
...
df_salida <- df_vict |>  # A la base de datos filtrada le aplico los procesamientos y creamos una nueva variable
  ...
  mutate ( ap6_4_01_r = ifelse( ap6_4_01 == 1 & ap6_5_01 == 1,1,0 ),  # En Robo de vehículo debemos confirmar si era del hogar o no.
           ...
           vic_res = rowSums( dplyr::across(preg) ), # Hago la suma de todas mis variables dummy
...
> A continuación, creamos un objeto de `diseño muestral` para poder realizar cálculos de muestras complejas.
...
print(paste0("Durante 2022 al menos un " , round (porc_hv,1), "% de hogares en Méxco tuvieron una víctima de delito." ))
